...
func TestIntegration_HelloWorld(t *testing.T) {
	...
	safe := confirmed + config.MinIncomingConfirmations - 1
...
func TestIntegration_RunLog(t *testing.T) {
	...
	defer cfgCleanup()
	config.MinOutgoingConfirmations = 6
	app, cleanup := cltest.NewApplicationWithConfig(config)
	defer cleanup()
	...
	minConfigHeight := logBlockNumber + int(app.Store.Config.MinOutgoingConfirmations)
...
func TestIntegration_SpecAndRunLog(t *testing.T) {
	config, cfgCleanup := cltest.NewConfig()
	defer cfgCleanup()
	config.MinOutgoingConfirmations = 3
	app, cleanup := cltest.NewApplicationWithConfig(config)
	defer cleanup()
...
func TestIntegration_NonceManagement_firstRunWithExistingTXs(t *testing.T) {
	...
	createCompletedJobRun := func(blockNumber uint64, expectedNonce uint64) {
		...
		confirmedBlockNumber := blockNumber + app.Store.Config.MinIncomingConfirmations
